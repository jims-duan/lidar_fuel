...
bool FastExplorationManager::getPathToPosition(const Vector3d& start, const Vector3d& goal,
                                               vector<Vector3d>& path_out) {
  planner_manager_->path_finder_->reset();
  if (planner_manager_->path_finder_->search(start, goal) != Astar::REACH_END)
    return false;
  path_out = planner_manager_->path_finder_->getPath();
  shortenPath(path_out);
